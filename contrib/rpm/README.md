...
The RPM spec file provided here is for Kobocoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://Kobocoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Kobocoin git repository](https://github.com/kobocoin/Kobocoin) but are not part of
...
Currently only `Source10` is used, for the example `Kobocoin.conf` file.
...
in the Kobocoin git repository. At present only an SVG version of the Kobocoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the Kobocoin
...
needed when 0.12.1 is released, a proper fix is already in the Kobocoin git
master branch.
...
idea to build Kobocoin Core against a static version of that library compiled
...
The `Kobocoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `Kobocoin-server` package. At this point, most RPM based
...
The `Kobocoin-server` package should have SELinux support. How to properly do
...
Kobocoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/Kobocoin/). He has been
packaging Kobocoin for Fedora at least since 2012.
...
4. I renamed the `Kobocoin` package that contains the Qt GUI to `Kobocoin-core` as
that appears to be how the general population refers to it, in contrast to
`Kobocoin-xt` or `Kobocoin-classic`. I wanted to make sure the general population
