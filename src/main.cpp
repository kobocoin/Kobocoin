...
{
    if (pindexBest->nHeight > (fTestNet ? 100 : 4000000)) {
        nTargetSpacing = 2 * 60;
	}
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (IsProofOfStake())
    {
    //m2:issue, need to stop past block being checked for stake reward on new chain.
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Kobocoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (fTestNet)
    {
        ...
        nCoinbaseMaturity = 50; // test maturity is 50 blocks
